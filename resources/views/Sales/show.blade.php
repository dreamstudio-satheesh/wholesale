...
<div class="content">
    ...
    <div class="invoice-wrapper rounded border bg-white py-5 px-3 px-md-4 px-lg-5">
        <div class="d-flex justify-content-between">
            <h2 class="text-dark font-weight-medium">Invoice #{{ $sale->invoice_number }}</h2>
        ...
        <div class="row pt-5">
            ...
            <div class="col-xl-4 col-lg-4">
                ...
                <address>
                    Invoice ID:
                    <span class="text-dark">#{{ $sale->invoice_number }}</span>
                    <br> {{ \Carbon\Carbon::parse($sale->date)->format('F d, Y') }}
                ...
                <table class="table table-bordered table-sm">
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <span id="display_discount">{{ config('settings.currency_symbol') }} {{$sale->discount_amount}} ({{ ucfirst($sale->discount_type) }})
                                </span>
                            </td>
                        </tr>
                        <tr>
                            <td class="bold">Tax </td>
                            <td id="display_tax">{{ config('settings.currency_symbol') }} {{$sale->tax_amount}} ({{$sale->tax_rate}}%)</td>
                        </tr>
                        <tr>
                            <td class="bold">Shipping</td>
                            <td id="display_shipping">{{ config('settings.currency_symbol') }} {{$sale->shipping_amount}}</td>
                        </tr>
                        <tr>
                            <td><span class="font-weight-bold">Grand Total</span></td>
                            <td class="font-weight-bold" id="grandTotal">{{ config('settings.currency_symbol') }} {{$sale->grand_total}}</td>
                        </tr>
                    </tbody>
