...
class ShowPaymentModal extends Component
{
    ...
    {
        $this->saleId = $saleId;
        $this->payments = SalesReturnPayment::where('sales_return_id', $saleId)->get();
    ...
    public function startEditingPayment($paymentId)
    {
        ...
        $payment = SalesReturnPayment::findOrFail($paymentId);
    ...
    public function savePayment()
    {
        ...
        DB::transaction(function () {
            $payment = SalesReturnPayment::find($this->editablePaymentId);
            if ($payment) {
                $payment->update([
            ...
            $sale = SalesReturn::with('payments')->find($this->saleId);
    ...
    public function deletePayment($paymentId)
    {
        DB::transaction(function () use ($paymentId) {
            $payment = SalesReturnPayment::find($paymentId);
            if ($payment) {
                ...
                $sale = SalesReturn::with('payments')->find($saleId);
                if ($sale) {
                    $totalPayments = $sale->payments->sum('amount');
                ...
                // $this->payments = SalesReturnPayment::where('sale_id', $saleId)->get();
            }
        });
    }
